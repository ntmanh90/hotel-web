...
export class DichVuComponent implements OnInit, OnDestroy {
  ...
  constructor(private DichVuService: DichVuService,  private modalService: NgbModal) {}
  ngOnInit(): void {
    this.loadAllDataDichVu();
  }
  // Get Products list
  private loadAllDataDichVu() {
    var sb = this.DichVuService.get_DanhSach().subscribe((data: {}) => {
  ...
  public openDialogChiTietDichVu(event){
    const modalRef = this.modalService.open(ChiTietDichVuComponent, { size: 'lg' }); //lg, xl, sm
  }
  ngOnDestroy()
  {
