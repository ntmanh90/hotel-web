...
import { LoaiPhong_TienIch_Requests } from "../../_models/loai-phong.model";
...
export class DialogThemTienIchComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ["select", "iD_TienIch", "tieuDe"];
  selection = new SelectionModel<TienIch>(true, []);
  constructor(
    private tienIchService: CommonService,
    public modal: NgbActiveModal
  ) {}
  private listAddTienIch: LoaiPhong_TienIch_Requests[] = [];
  ...
  onClickCheckbox(row?: TienIch) {
    if (
      this.listAddTienIch.indexOf({
        iD_TienIch: row.iD_TienIch,
        tieuDe: row.tieuDe,
      } as LoaiPhong_TienIch_Requests) === -1
    ) {
      this.listAddTienIch.push({
        iD_TienIch: row.iD_TienIch,
        tieuDe: row.tieuDe,
        check: true,
      });
    } else {
      this.listAddTienIch.splice(
        this.listAddTienIch.indexOf({
          iD_TienIch: row.iD_TienIch,
          tieuDe: row.tieuDe,
        } as LoaiPhong_TienIch_Requests),
        1
      );
    }
  }
